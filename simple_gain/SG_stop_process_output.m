% model_params = SG_stop_process_output(model_params)
%
...
function model_params = SG_stop_process_output(model_params)

%% Get the Avalon streaming signals from the model
t = model_params.Avalon_Sink_Data.Time;             % time
d = squeeze(model_params.Avalon_Sink_Data.Data);    % data      Note: the Matlab squeeze() function removes singleton dimensions (i.e. dimensions of length 1)
c = squeeze(model_params.Avalon_Sink_Channel.Data); % channel
v = squeeze(model_params.Avalon_Sink_Valid.Data);   % valid
left_index = 1;
right_index = 1;
for i=1:length(v)
    if v(i) == 1  % check if valid, valid is asserted when there is data
        if c(i) == 0  % if the channel number is zero, it is left channel data
            model_params.left_data_out(left_index) = double(d(i));
            model_params.left_time_out(left_index) = t(i);
            left_index            = left_index + 1;
        end
        if c(i) == 1  % if the channel number is one, it is right channel data
            model_params.right_data_out(right_index) = double(d(i));
            model_params.right_time_out(right_index) = t(i);
