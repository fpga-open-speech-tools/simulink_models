...
Register_Write_En = [tt_system, zeros(length(tt_system), 1)];
% Filtering Signal
inSignal = [tt_system, cos(tt_system)]; 


%% Here, we mess around with model inputs for testing purposes.
% first, set up the registers to fill the tables.
i = 1;
for ix = 2^ram_size : -1 : 1
    % first lookup table is a forward count
    Register_Data(i,2) = i;
    Register_Addr(i,2) = i-1;
    Register_Write_En(i,2) = 1;
    i = i+1;
end

% for ix = 2^ram_size : -1 : 1
%     % Second lookup table is a forward count
