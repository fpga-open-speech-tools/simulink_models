...
static struct of_device_id fe_noise_suppression_dt_ids[] = {
  {
    .compatible = "dev,fe-noise_suppression"
  },
  { }
};
...
static int noise_suppression_init(void) {
  int ret_val = 0;
  printk(KERN_ALERT "FUNCTION AUTO GENERATED AT: 2020-07-08 20:19\n");
...
static int noise_suppression_probe(struct platform_device *pdev) {
  int ret_val = -EBUSY;
  char deviceName[29] = "fe_noise_suppression_";
  char deviceMinor[20];
  int status;
...
/* Beginning attribute file stuff */
  ...
  status = device_create_file(device_obj, &dev_attr_noise_variance);
  if (status)
...
bad_device_create_file_1:
  device_remove_file(device_obj, &dev_attr_noise_variance);
...
static ssize_t noise_variance_read(struct device *dev, struct device_attribute *attr, char *buf) {
  fe_noise_suppression_dev_t * devp = (fe_noise_suppression_dev_t *)dev_get_drvdata(dev);
  fp_to_string(buf, devp->noise_variance, 31, false, 32);
  strcat2(buf,"\n");
  return strlen(buf);
}
...
static ssize_t noise_variance_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count) {
  ...
  substring[substring_count] = 0;
  tempValue = set_fixed_num(substring, 31, false);
