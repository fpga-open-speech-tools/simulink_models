-- -------------------------------------------------------------
-- 
-- File Name: hdlsrc\DynamicCompressionModel\Compression_Gain_Calc.vhd
...
-- -------------------------------------------------------------
-- 
-- Module: Compression_Gain_Calc
-- Source Path: DynamicCompressionModel/recalculate/Nchan_FbankAGC_AID/Compression_1/Compression_Gain_Calc
-- Hierarchy Level: 3
-- 
-- -------------------------------------------------------------
LIBRARY IEEE;
...
ENTITY Compression_Gain_Calc IS
...
END Compression_Gain_Calc;


ARCHITECTURE rtl OF Compression_Gain_Calc IS

  -- Component Declarations
  COMPONENT LogAddressing
    PORT( clk                             :   IN    std_logic;
          reset                           :   IN    std_logic;
          enb                             :   IN    std_logic;
  ...
  COMPONENT Linear_Approximation
    PORT( clk                             :   IN    std_logic;
          reset                           :   IN    std_logic;
          enb                             :   IN    std_logic;
  ...
  FOR ALL : LogAddressing
    USE ENTITY work.LogAddressing(rtl);

  FOR ALL : High_Gain_Table
    USE ENTITY work.High_Gain_Table(rtl);

  FOR ALL : Low_Gain_Table
    USE ENTITY work.Low_Gain_Table(rtl);

  FOR ALL : Linear_Approximation
    USE ENTITY work.Linear_Approximation(rtl);
...
BEGIN
  ...
  u_High_Gain_Table : High_Gain_Table
    PORT MAP( clk => clk,
              reset => reset,
              enb => enb,
  ...
  u_Low_Gain_Table : Low_Gain_Table
    PORT MAP( clk => clk,
              reset => reset,
              enb => enb,
              wr_din => Table_Fill,  -- sfix32_En28
  ...
  u_Linear_Approximation : Linear_Approximation
    PORT MAP( clk => clk,
              reset => reset,
              enb => enb,
