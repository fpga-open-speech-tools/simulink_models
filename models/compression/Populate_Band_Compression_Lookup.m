function [Y_lookup_out] = Populate_Band_Compression_Lookup ...
...
%       Inputs shoud be logorithmically spaced, as the FPGA fabric expects
%       logarithmically spaced data points for Y_out hash function.
%       Bounded from -X_in_max <= Xp_in <= X_in_max
%
...
Y_lookup_out = Comp_Threshold.*nthroot(abs(Xp_in)./Comp_Threshold , Comp_Ratio);
% set all x's (before prescription) less than the threshold amplitude to 0
x_zero = find(Xp_in < Comp_Threshold);
Y_lookup_out(x_zero) = 10^-8;
end