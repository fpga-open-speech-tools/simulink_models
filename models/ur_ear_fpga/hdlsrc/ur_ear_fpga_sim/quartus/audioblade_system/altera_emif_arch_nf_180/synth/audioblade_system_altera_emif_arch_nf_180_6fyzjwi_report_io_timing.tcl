...
source "$script_dir/audioblade_system_altera_emif_arch_nf_180_6fyzjwi_report_timing_core.tcl"


if [ info exists ddr_db ] {
   unset ddr_db
}
audioblade_system_altera_emif_arch_nf_180_6fyzjwi_initialize_ddr_db ddr_db var
...
foreach inst $instances {
   ...
   if {[llength $instances] <= 1} {
      set fbasename "${::GLOBAL_audioblade_system_altera_emif_arch_nf_180_6fyzjwi_corename}"
   } else {
      set fbasename "${::GLOBAL_audioblade_system_altera_emif_arch_nf_180_6fyzjwi_corename}_${inst_id}"
   }
   ...
   audioblade_system_altera_emif_arch_nf_180_6fyzjwi_perform_read_capture_analysis $opcname $inst pins var summary
   
   #######################################
   # Write Analysis

   audioblade_system_altera_emif_arch_nf_180_6fyzjwi_perform_write_launch_analysis $opcname $inst pins var summary

   #######################################
   # Address/command Analyses

   audioblade_system_altera_emif_arch_nf_180_6fyzjwi_perform_ac_analysis  $opcname $inst pins var summary

   #######################################
   # DQS Gating Analysis

   audioblade_system_altera_emif_arch_nf_180_6fyzjwi_perform_dqs_gating_analysis   $opcname $inst pins var summary

   #######################################
   # Write Levelling Analysis 
   
   audioblade_system_altera_emif_arch_nf_180_6fyzjwi_perform_write_levelling_analysis $opcname $inst pins var summary

   #######################################
   # PHY Analyses
   
   audioblade_system_altera_emif_arch_nf_180_6fyzjwi_perform_core_analysis $opcname $inst pins var summary

   #######################################
   # Print out the Summary Panel for this instance   

   set summary [lsort -command audioblade_system_altera_emif_arch_nf_180_6fyzjwi_sort_proc $summary]
   ...
   puts $f "Core: ${::GLOBAL_audioblade_system_altera_emif_arch_nf_180_6fyzjwi_corename} - Instance: $inst"
   puts $f "Path, Setup Margin, Hold Margin"

   
   post_message -type info "Core: ${::GLOBAL_audioblade_system_altera_emif_arch_nf_180_6fyzjwi_corename} - Instance: $inst"
   ...
   foreach summary_line $summary {
      ...
      if {$su != "--"} {
         set su [ audioblade_system_altera_emif_arch_nf_180_6fyzjwi_round_3dp $su]
      }
      if {$hold != "--"} {
         set hold [ audioblade_system_altera_emif_arch_nf_180_6fyzjwi_round_3dp $hold]
      }
