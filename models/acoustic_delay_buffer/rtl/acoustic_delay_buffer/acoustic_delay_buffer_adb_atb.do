...
if {! [file exists $base_dir/acoustic_delay_buffer_adb]} {
    file delete -force $base_dir/acoustic_delay_buffer_adb
}

project new $base_dir acoustic_delay_buffer_adb
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb_fpc.do"
} else {
...
vcom -quiet $base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb.vhd
vcom -quiet $base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb_AStInput.vhd
vcom -quiet $base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb_AStOutput.vhd
...
vcom -quiet $base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb_delay_buffer.vhd
vcom -quiet -O0 $base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb_atb.vhd
vcom -quiet $base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_dout} mismatch] == 0 && $mismatch } {
                report_mismatch dout $cyclenum
            }
            if { [catch {exa mismatch_vout} mismatch] == 0 && $mismatch } {
                report_mismatch vout $cyclenum
            }
            if { [catch {exa mismatch_cout} mismatch] == 0 && $mismatch } {
                report_mismatch cout $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473  -voptargs="+acc" -t ps acoustic_delay_buffer_adb_atb 
    do $base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb_atb.wav.do
