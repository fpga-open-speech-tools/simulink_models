...
if {! [file exists $base_dir/acoustic_delay_buffer_adb_AStOutput]} {
    file delete -force $base_dir/acoustic_delay_buffer_adb_AStOutput
}

project new $base_dir acoustic_delay_buffer_adb_AStOutput
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb_AStOutput_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb_AStOutput_atb.vhd
vcom -quiet $base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb_AStOutput_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_avalonIfaceRole_valid_source_valid} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_avalonIfaceRole_valid_source_valid $cyclenum
            }
            if { [catch {exa mismatch_out_2_avalonIfaceRole_channel_source_channel} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_avalonIfaceRole_channel_source_channel $cyclenum
            }
            if { [catch {exa mismatch_out_3_avalonIfaceRole_data_source_data} mismatch] == 0 && $mismatch } {
                report_mismatch out_3_avalonIfaceRole_data_source_data $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473  -voptargs="+acc" -t ps acoustic_delay_buffer_adb_AStOutput_atb 
    do $base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb_AStOutput_atb.wav.do
