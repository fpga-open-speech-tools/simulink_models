...
if {! [file exists $base_dir/acoustic_delay_buffer_adb_delay_buffer]} {
    file delete -force $base_dir/acoustic_delay_buffer_adb_delay_buffer
}

project new $base_dir acoustic_delay_buffer_adb_delay_buffer
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb_delay_buffer_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb_delay_buffer_atb.vhd
vcom -quiet $base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb_delay_buffer_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_valid_out} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_valid_out $cyclenum
            }
            if { [catch {exa mismatch_out_2_channel_out} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_channel_out $cyclenum
            }
            if { [catch {exa mismatch_out_3_dout} mismatch] == 0 && $mismatch } {
                report_mismatch out_3_dout $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473  -voptargs="+acc" -t ps acoustic_delay_buffer_adb_delay_buffer_atb 
    do $base_dir/acoustic_delay_buffer/acoustic_delay_buffer_adb_delay_buffer_atb.wav.do
