...
if {! [file exists $base_dir/dummy_beamformer_ch_sum_ch_sum]} {
    file delete -force $base_dir/dummy_beamformer_ch_sum_ch_sum
}

project new $base_dir dummy_beamformer_ch_sum_ch_sum
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/dummy_beamformer/dummy_beamformer_ch_sum_ch_sum_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/dummy_beamformer/dummy_beamformer_ch_sum_ch_sum_atb.vhd
vcom -quiet $base_dir/dummy_beamformer/dummy_beamformer_ch_sum_ch_sum_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_2_vout} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_vout $cyclenum
            }
            if { [catch {exa mismatch_out_3_cout} mismatch] == 0 && $mismatch } {
                report_mismatch out_3_cout $cyclenum
            }
            if { [catch {exa mismatch_out_1_dout} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_dout $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473  -voptargs="+acc" -t ps dummy_beamformer_ch_sum_ch_sum_atb 
    do $base_dir/dummy_beamformer/dummy_beamformer_ch_sum_ch_sum_atb.wav.do
