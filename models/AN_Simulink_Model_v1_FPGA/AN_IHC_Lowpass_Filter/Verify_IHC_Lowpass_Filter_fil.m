...
subplot(2,1,1)
plot(out.ihcout);
title('Inner Hair Cell Lowpass Filter Output - Simulink Single Precision')
xlabel('Sample Number');
subplot(2,1,2)
plot(out.ihcoutfpga)
title('Inner Hair Cell Lowpass Filter Output - FPGA-in-the-Loop')
xlabel('Sample Number');
...
[avgpercerror,maxpercerror,normerror,percerror] = calcerror(out.ihcout, out.ihcoutfpga);

% Adding more detailed title to function generated Error Distribution Plot
title('Percent Error Distribution (Stage 2) - IHC Lowpass Filter');

% Display in Simulink Diagnostics menu
disp('IHC Lowpass Filter Average Percent Error =');
disp(avgpercerror);

% Display in Simulink Diagnostics menu
disp('IHC Lowpass Filter Maximum Percent Error =');
disp(maxpercerror);

% Display in Simulink Diagnostics menu
disp('IHC Lowpass Filter Euclidean Norm Error =');
disp(normerror);
